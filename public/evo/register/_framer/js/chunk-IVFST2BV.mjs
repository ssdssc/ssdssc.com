...
var PresenceChild = ({ children, initial, isPresent: isPresent2, onExitComplete, custom, presenceAffectsLayout, mode, anchorX, anchorY, root }) => {
  ...
  const id4 = useId2();
  const isPresentRef = useRef3(isPresent2);
...
import { useContext as useContext4, useRef as useRef6, useMemo as useMemo3 } from "react";
...
import { useState as useState3, useRef as useRef7, useEffect as useEffect4 } from "react";
...
function LazyMotion({ children, features, strict = false }) {
  ...
  const loadedRenderer = useRef7(void 0);
...
import { useRef as useRef8, useInsertionEffect as useInsertionEffect2, useCallback as useCallback4 } from "react";
function useMotionRef(visualState, visualElement, externalRef) {
  const externalRefContainer = useRef8(externalRef);
  useInsertionEffect2(() => {
    externalRefContainer.current = externalRef;
  });
  const refCleanup = useRef8(null);
...
import { useContext as useContext8, useRef as useRef9, useInsertionEffect as useInsertionEffect3, useEffect as useEffect5 } from "react";
...
function useVisualElement(Component3, visualState, props, createVisualElement2, ProjectionNodeConstructor, isSVG) {
  ...
  const visualElementRef = useRef9(null);
  const hasMountedOnce = useRef9(false);
  ...
  const wantsHandoff = useRef9(Boolean(optimisedAppearId) && typeof window !== "undefined" && !window.MotionHandoffIsComplete?.(optimisedAppearId) && window.MotionHasOptimisedAnimation?.(optimisedAppearId));
  useIsomorphicLayoutEffect(() => {
...
import { useRef as useRef10, useCallback as useCallback5, useEffect as useEffect7 } from "react";
...
function useScroll({ container, target, ...options } = {}) {
  ...
  const scrollAnimation = useRef10(null);
  const needsStart = useRef10(false);
...
import { useRef as useRef11, useContext as useContext13, useEffect as useEffect9 } from "react";
...
import { useRef as useRef12, useState as useState6, useCallback as useCallback7 } from "react";
...
import { useRef as useRef13, useEffect as useEffect12 } from "react";
function useInstantTransition() {
  const [forceUpdate, forcedRenderCount] = useForceUpdate();
  const startInstantLayoutTransition = useInstantLayoutTransition();
  const unlockOnFrameRef = useRef13(-1);
  useEffect12(() => {
...
import { forwardRef as forwardRef2, useRef as useRef14, useEffect as useEffect15 } from "react";
...
function ReorderGroupComponent({ children, as = "ul", axis = "y", onReorder, values, ...props }, externalRef) {
  ...
  const order = [];
  const isReordering = useRef14(false);
  const groupRef = useRef14(null);
...
//# sourceMappingURL=chunk-IVFST2BV.mjs.map
